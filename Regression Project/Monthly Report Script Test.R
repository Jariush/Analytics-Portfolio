...
# Prompt user for the folder path and route number
folder_path <- readline(prompt = "Enter the folder path containing the .xlsx files: ")
route <- readline(prompt = "Enter the route number: ")

# Define the fixed path to the output CSV file
...
extract_year_month_from_filename <- function(filename) {
  ...
  if (length(parts) >= 3) {
    ...
    month_num <- as.numeric(parts[3])  # The third part should be the month
